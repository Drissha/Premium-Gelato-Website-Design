...
const isGelatoProduct = (product: NormalizedProduct) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const gelatoItems = items.filter(isGelatoProduct);

        if (isActive) {
          setProducts(gelatoItems);
        }
      } catch (fetchError) {
        if (isActive) {
          setError(fetchError instanceof Error ? fetchError.message : 'Failed to load gelato products.');
          setProducts([]);
        }
      } finally {
        if (isActive) {
      ...
      <div className="max-w-7xl sm:mt-20 mx-auto px-6 lg:px-8">
        ...
        >
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search gelato flavors..."
              value={searchQuery}
        ...
        >
          Showing <span className="font-bold text-blue-reguler">{filteredProducts.length}</span> gelato products
        </motion.div>
        ...
        {!loading && filteredProducts.length === 0 && (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-2">No gelato found</h3>
